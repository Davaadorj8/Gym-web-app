...
import { ClientService } from '@/modules/clients/server';
import { WorkoutService } from '@/modules/workouts/server';
...
export class StatsService {
  static async getDashboardStats(userRole: string = 'ADMIN'): Promise<DashboardStats> {
    ...
    if (isDatabaseConfigured()) {
      try {
        const [totalClients, activeClients, pendingClients, totalPlans, sessionCount] =
          await Promise.all([
            prisma.client.count(),
        ...
        return {
          ...
          avgRetentionRate:
            totalClients > 0 ? Math.round((activeClients / totalClients) * 100) : 94,
  ...
  static async getOverviewStats(): Promise<DashboardStats> {
    return this.getDashboardStats('ADMIN');
  }
}
