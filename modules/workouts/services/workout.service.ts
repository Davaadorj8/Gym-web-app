...
let fallbackWorkouts: WorkoutPlan[] = [
  {
    ...
    description:
      'High-density hypertrophy program focusing on compound lifts, progressive overload, and eccentric control.',
    level: 'INTERMEDIATE',
    ...
    exercises: [
      ...
      },
      {
        ...
        restSeconds: 60,
        orderIndex: 4,
        notes: 'Hold 1s at peak contraction',
      },
    ],
  ...
  },
  {
    ...
    description:
      'Zone 2 & Zone 4 intervals for endurance athletes, hybrid trainees, and metabolic capacity development.',
    level: 'ADVANCED',
    category: 'Endurance',
    durationWeeks: 6,
    isPublished: true,
    ...
    exercises: [
      {
        id: 'ex-5',
        planId: 'wp-2',
        name: 'Row Ergometer Intervals',
        targetMuscle: 'Full Body',
        sets: 6,
        reps: 500,
        restSeconds: 60,
        orderIndex: 1,
        notes: '500m per split at 1:45 pace',
      },
      {
        id: 'ex-6',
        planId: 'wp-2',
  ...
  },
  {
    ...
    description:
      'Low-impact movement protocol for clients returning from injury or sedentary desk work.',
    level: 'BEGINNER',
    category: 'Mobility',
    durationWeeks: 4,
    isPublished: true,
    ...
    exercises: [
      {
        id: 'ex-8',
        planId: 'wp-3',
        name: 'Cat-Cow Flow & Thoracic Rotations',
        targetMuscle: 'Thoracic Spine',
        sets: 3,
        reps: 12,
        restSeconds: 30,
        orderIndex: 1,
        notes: 'Smooth diaphragmatic breathing',
      },
      {
        id: 'ex-9',
        planId: 'wp-3',
        name: '90/90 Hip Switch & Lift',
        targetMuscle: 'Hip Capsule',
        sets: 3,
        reps: 10,
        restSeconds: 30,
        orderIndex: 2,
      ...
      },
      {
        ...
        restSeconds: 45,
        orderIndex: 3,
        notes: 'Lower back glued to floor',
      },
    ],
  ...
  static async getPlanById(id: string): Promise<WorkoutPlan | null> {
    if (isDatabaseConfigured()) {
      try {
        ...
        console.warn('Prisma plan lookup failed, falling back to local memory:', err);
      }
    }

    return fallbackWorkouts.find((p) => p.id === id) || null;
  }

  static async createPlan(data: WorkoutPlanInput, trainerId: string = 'u-1'): Promise<WorkoutPlan> {
    if (isDatabaseConfigured()) {
      try {
        ...
        console.warn('Prisma plan creation failed, adding to fallback state:', err);
      }
    }
  ...
  static async deletePlan(id: string): Promise<boolean> {
    if (isDatabaseConfigured()) {
      try {
        ...
        console.warn('Prisma plan deletion failed:', err);
      }
    }

    fallbackWorkouts = fallbackWorkouts.filter((p) => p.id !== id);
    return true;
  }
}
